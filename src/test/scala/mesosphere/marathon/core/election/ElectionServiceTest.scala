...
class ElectionServiceTest extends AkkaUnitTest with Eventually {
  trait Fixture {
    ...
    val stubElectionCandidate: ElectionCandidate = new ElectionCandidate {
      override def startLeadership(cancellable: Cancellable) = Future.successful(Done)
      override def stopLeadership() = Future.successful(Done)
    }
    val electionService = new ElectionServiceImpl(
      "127.0.0.1:2015",
      leaderEvents,
      crashStrategy,
      stubElectionCandidate
    )(system)
  ...
  "it yields a LeadershipTransition event when going from standby to leader" in new Fixture {
    ...
    input.offer(LeadershipState.Standby(None))
    input.offer(LeadershipState.ElectedAsLeader)
    transitionEvent.futureValue shouldBe LeadershipTransition.LeaderElectedAndReady
  }
  ...
  "it provides the last leadershipEvent published for streams showing up late" in new Fixture {
    ...
    input.offer(LeadershipState.Standby(None))
    lostEvent.futureValue shouldBe Seq(LeadershipTransition.LeaderElectedAndReady, LeadershipTransition.LostLeadership)
