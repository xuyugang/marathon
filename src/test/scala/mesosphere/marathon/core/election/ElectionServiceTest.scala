...
class ElectionServiceTest extends AkkaUnitTest with Eventually {
  trait Fixture {
    ...
    val leaderTransitionEvents: Source[LeadershipTransition, Cancellable] = Source.actorRef(16, OverflowStrategy.fail)
      .watchTermination()(Keep.both)
      .mapMaterializedValue {
        case (ref, finished) =>
          electionService.subscribe(ref)
          val cancellable = new CancellableOnce(() => ref ! PoisonPill)
          finished.onComplete(_ => cancellable.cancel())
          cancellable
      }
  }
