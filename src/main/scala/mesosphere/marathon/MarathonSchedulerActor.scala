...
class MarathonSchedulerActor private (
  ...
  leaderTransitionEvents: Source[LeadershipTransition, Cancellable],
  ...
  override def preStart(): Unit = {
    ...
    stateSubscription = leaderTransitionEvents
      .to(Sink.foreach { leaderTransition => self ! leaderTransition })
      .run
  }
  ...
  def started: Receive = LoggingReceive.withLabel("started") {
    ...
    case LeadershipTransition.LeaderElectedAndReady => // we shouldn't receive this here; ignore
